...
// codes referenced from Mirza Ramadhany re-adapt by me Andi Dwi
...
class ezDB extends PDO {
   ...
   private $qShowError  = true;
    ...
    public function Update($cTable,$vaData=array(),$cWhere="",$lUpdateLog=true,$vaInsert=array(),$vaEdit=array() ){
        $dbData = $this->GetData($cTable,"*",$cWhere) ; 
    ...
    public function SaveLog($cTable,$cTipe,$cSql){ 
        if($this->qLog == "db_log"){
            ...
            $vaData = array("tbl"=>$cTable,"thistime"=>date("Y-m-d H:i:s"),  
                            ...
                            "username"=>$_SESSION['username'],"thissql"=>$cSqla) ;  
            $this->Insert("log_sql_".date("Ym"),$vaData,false) ;    
        }else if($this->qLog == "db"){  
            $cSqla  = htmlspecialchars($cSql) ;//str_replace("'"," ",$cSql) ; //agar dapat disave
